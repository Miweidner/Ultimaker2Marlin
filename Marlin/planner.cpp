...
void plan_buffer_step(const long &x, const long &y, const long &z, const long &e, float feed_rate, const uint8_t &extruder)
{
  // Calculate the buffer head after we push this byte
  int next_buffer_head = next_block_index(block_buffer_head);
  ...
  //this should be done after the wait, because otherwise a M92 code within the gcode disrupts this calculation somehow
  long target[4];
  target[X_AXIS] = st_get_position(X_AXIS) + x;
  target[Y_AXIS] = st_get_position(Y_AXIS) + y;
  target[Z_AXIS] = st_get_position(Z_AXIS) + z;
  target[E_AXIS] = st_get_position(E_AXIS) + e;
